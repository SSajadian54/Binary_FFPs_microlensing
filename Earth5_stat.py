...
f1=open("./files/MONT/MonteEarth5.txt","r")
nm= sum(1 for line in f1) 
arr= np.zeros((nm,52))
arr=np.loadtxt("./files/MONT/MonteEarth5.txt")
fil1=open('./EarthPap5.txt',"w"); 
fil2=open('./files/MONT/Earth5.txt',"w"); 
...
def histo(array, i,  flag): 
    ...
    if(flag==0):  
        ...
        fig3.savefig("./lights/Histo/Earth5/Total_{0:d}.jpg".format(i) ,dpi=200)
    if(flag==1):  
        plt.xlabel(str(nam2[i]),fontsize=18,labelpad=0.1)
        fig3.savefig("./lights/Histo/Earth5/Detect_{0:d}.jpg".format(i) , dpi=200)
    return(0)    
########################################################################      
def histo2(array1, array0, i):
    ...
    plt.grid(linestyle='dashed')
    fig3= plt.gcf()
    fig3.savefig("./lights/Histo/Earth5/Both_{0:d}.jpg".format(i) , dpi=200)
    return(0)        
########################################################################  
'''
def Eroman(mag):
...
for i in range(nm): 
    ...
    if(ros>0.0):
        ...
        fil2=open('./files/MONT/Earth5.txt',"a")
...
########################################################################
    sav=np.array([5.0, ave[0,0],ave[0,1],  ave[1,0],ave[1,1], np.log10(ave[3,0]),np.log10(ave[3,1]),  
