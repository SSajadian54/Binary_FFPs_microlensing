...
nam=[r"$\theta(\rm{degree})$", r"$,~~~\Delta \rm{FWHM}(t_{\rm E})$",  r"$,~~~\Delta \chi^{2}$"]
...
for j in range(24): 
    teta=float(j*7.5*np.pi/180.0)
    chi1=0.0; chi2=0.0;  time=0.0; k=0;     
    for i in range (ntim):
        tim[i]=float(-2.5*tE+ i*dt)/tE
        traj[i,0]= tim[i]*np.cos(teta) -u0*np.sin(teta) +x0
        traj[i,1]= tim[i]*np.sin(teta) +u0*np.cos(teta) +y0  
...
##############################################################   
    ...
    ax1.set_title(str(nam[0])+ '={0:.1f}'.format(teta*180.0/np.pi) + str(nam[1])+ '={0:.2f}'.format(float(FWHM1-FWHM0)) +str(nam[2])+ '={0:.1f}'.format(dchi),fontsize=15,color='k')
    plt.grid(True)
    ...
    ax3.plot(xsou+x0, ysou+y0, "k-", lw=1.2)
    ...
    fig=plt.gcf()
    fig=plt.savefig("./lights/planetary_theta/LIc_{0:d}.jpg".format(j),dpi=200)
    plt.clf()
    plt.cla()
    py.clf()
