...
nam=[r"$\rho_{\star}$", r"$,~~~\Delta \rm{FWHM}(t_{\rm E})$",  r"$,~~~\Delta \chi^{2}$"]
...
##############################################################   
    ...
    ax1.set_title(str(nam[0])+ '={0:.1f}'.format(ros) + str(nam[1])+ '={0:.2f}'.format(float(FWHM1-FWHM0)) +str(nam[2])+ '={0:.1f}'.format(dchi),fontsize=15,color='k')
    plt.grid(True)
    ...
    fig=plt.gcf()
    fig=plt.savefig("./lights/central_rho/LIc_{0:d}.jpg".format(j),dpi=200)
    plt.clf()
    plt.cla()
    py.clf()
